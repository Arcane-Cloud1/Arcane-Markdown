...
import { 
  ...
  Table, CheckSquare, Sigma, Network, FoldVertical
} from "lucide-react";
...
import { MermaidBuilder, MermaidData } from "@/components/markdown/MermaidBuilder";
...
const parseMarkdown = (md: string): Block[] => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (currentBlock && (currentBlock as Block).type.startsWith("callout-")) {
        ...
        if (line.startsWith("> ")) {
            (currentBlock as Block).content += ((currentBlock as Block).content ? "\n" : "") + line.substring(2);
            continue;
        }
    ...
    if (line.startsWith("- [ ] ")) { blocks.push({ id: generateId(), type: "checklist", content: line.substring(6), metadata: { checked: false } }); continue; }
    if (line.startsWith("- [x] ")) { blocks.push({ id: generateId(), type: "checklist", content: line.substring(6), metadata: { checked: true } }); continue; }
    ...
    if (line.startsWith("$$")) {
        ...
        if (line.trim() === "$$") {
            ...
            continue;
             ...
             // Inline block math $$...$$
             blocks.push({ id: generateId(), type: "math", content: line.replace(/^\$\$/, "").replace(/\$\$$/, "") });
             continue;
        }
    }
    ...
    if (line.startsWith("<details>")) {
        ...
        blocks.push({ id: generateId(), type: "details", content: detailsContent.trim(), metadata: { summary } });
        i = j;
        continue;
    }
    if (line.startsWith("```")) { 
        ...
        blocks.push({ id: generateId(), type: "code", content: codeContent.trim(), metadata: { language } });
        i = j;
        continue;
    }
    ...
    if (line.trim().startsWith("|")) {
        ...
        if (tableLines.length > 0) {
            ...
            if (data.length > 0) {
                ...
                blocks.push({ id: generateId(), type: "table", content: "", metadata: { data: normalizedData } });
                i = j - 1;
                continue;
            }
        }
...
const serializeMarkdown = (blocks: Block[]): string => {
  return blocks.map(block => {
    switch (block.type) {
      ...
      case "checklist": return `- [${block.metadata?.checked ? "x" : " "}] ${block.content}`;
      case "divider": return `---`;
...
const RichBlockTextarea = ({ value, onChange, placeholder, className }: { 
    value: string, 
    ...
    return (
        <div className="group/rich relative">
            <div className="absolute -top-10 left-0 z-10 hidden group-focus-within/rich:block">
                <div className="bg-white border rounded-md shadow-lg p-1">
...
export function ArcaneEditor({ initialData, isEditing = false }: ArcaneEditorProps) {
  ...
  const modules = [
    { category: t.editor.modules.basics, items: [
    ...
    { category: t.editor.modules.lists, items: [
    ...
    { category: t.editor.modules.mediaAndStructure, items: [
    ...
    { category: t.editor.modules.components, items: [
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-20">
      ...
      <div className="flex items-center justify-between sticky top-0 bg-gray-50/95 backdrop-blur z-10 py-4 border-b">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-bold">{isEditing ? t.editor.editTitle : t.editor.newTitle}</h1>
        </div>
            ...
            >
                ...
                {groupBlocks(blocks).map((group) => {
                  if (group.type === "group") {
                    let olCounter = 0;
                    return (
                        <div key={group.id} className="group relative flex gap-2 items-start">
                            ...
                            <div className="absolute -left-12 top-2 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                                <button onClick={() => removeBlock(group.blocks[0].id)} className="p-1.5 bg-white border shadow-sm rounded-md hover:bg-red-50 text-red-500" title={t.common.delete}><Trash2 className="h-3 w-3" /></button>
                            </div>

                            <div className="flex-1 bg-white border border-transparent hover:border-blue-200 rounded-lg p-3 transition-colors ring-offset-2 focus-within:ring-2 ring-blue-500/20 space-y-1">
                                {group.blocks.map((block, bIndex) => {
                                    ...
                                    return (
                                        <div key={block.id} className="relative flex gap-2 items-start group/item">
                                            {/* Item Controls (Hover only) */}
                                            <div className="absolute -left-8 top-1 opacity-0 group-hover/item:opacity-100 transition-opacity flex gap-1">
                                                 <div className="flex flex-col">
                                                    <button onClick={() => moveBlock(block.id, "up")} className="p-0.5 hover:bg-gray-100 text-gray-400 hover:text-gray-600"><ArrowUp className="h-3 w-3" /></button>
                                                    <button onClick={() => moveBlock(block.id, "down")} className="p-0.5 hover:bg-gray-100 text-gray-400 hover:text-gray-600"><ArrowDown className="h-3 w-3" /></button>
                                                 </div>
                                                 <button onClick={() => removeBlock(block.id)} className="p-1 hover:bg-red-50 text-gray-300 hover:text-red-500"><Trash2 className="h-3 w-3" /></button>
                                            </div>
                                            ...
                                            {block.type === "checklist" ? (
                                                ...
                                                <span className="mt-2 text-gray-400 font-mono select-none w-6 text-right shrink-0">
                                            ...
                                            <Input 
                                                ...
                                                placeholder={block.type === "checklist" ? t.editor.placeholders.checklist : t.editor.placeholders.list}
                                                className={cn(
                                                    "border-none shadow-none focus-visible:ring-0 p-0 h-auto py-1 flex-1 min-w-0",
                                                    block.type === "checklist" && block.metadata?.checked && "text-gray-400 line-through"
                                                )}
                        ...
                        <div className="flex-1 bg-white border border-transparent hover:border-blue-200 rounded-lg p-3 transition-colors ring-offset-2 focus-within:ring-2 ring-blue-500/20">
                            ...
                            {block.type === "code" && (
                                <div className="bg-gray-900 rounded-md overflow-hidden font-mono text-sm">
                                    <div className="flex justify-between items-center px-4 py-2 bg-gray-800 text-gray-400 text-xs uppercase">
                            ...
                            {block.type === "math" && (
                                <div className="bg-gray-50 border rounded-md p-4">
                                    <div className="flex justify-between items-center mb-2 text-gray-500 text-xs uppercase">
                                        <span className="flex items-center gap-1"><Sigma className="h-3 w-3" /> {t.editor.blocks.math} (LaTeX)</span>
                                    </div>
                                    <Textarea 
                                        value={block.content} 
                                        onChange={(e) => updateBlock(block.id, e.target.value)}
                                        placeholder={t.editor.placeholders.math}
                                        className="min-h-[60px] border-none shadow-none resize-none focus-visible:ring-0 p-0 font-mono text-gray-800 bg-transparent"
                                    />
                                </div>
                            )}
                            {block.type === "mermaid" && (
                                <div className="bg-slate-50 border border-slate-200 rounded-md p-4">
                                    <div className="flex justify-between items-center mb-4 text-slate-500 text-xs uppercase">
                                        <span className="flex items-center gap-1"><Network className="h-3 w-3" /> {t.editor.blocks.mermaid}</span>
                                    </div>
                                    <MermaidBuilder 
                                        initialData={block.metadata?.mermaidData}
                                        onUpdate={(code, data) => updateBlock(block.id, code, { mermaidData: data })}
                                    />
                                </div>
                            )}
                            {block.type === "details" && (
                                <div className="border rounded-md p-4 space-y-2 bg-white">
                                    ...
                                    <Textarea 
                                        ...
                                        className="min-h-[80px] border-none shadow-none resize-none focus-visible:ring-0 p-0 text-gray-600"
                                    />
                                </div>
                            )}
                            {block.type === "table" && (
                                <div className="overflow-x-auto">
                                    {(() => {
                                        const data = block.metadata?.data || [["Header 1", "Header 2"], ["Cell 1", "Cell 2"]];
                                        const updateTable = (newData: string[][]) => updateBlock(block.id, "", { data: newData });
                                        ...
                                        const removeRow = (idx: number) => {
                                            ...
                                            updateTable(data.filter((_: any, i: number) => i !== idx));
                                        };
                                        const removeCol = (idx: number) => {
                                            if (data[0].length <= 1) return;
                                            updateTable(data.map((row: string[]) => row.filter((_: any, i: number) => i !== idx)));
                                        };
                                        ...
                                        return (
                                            <div className="space-y-2">
                                                    ...
                                                    <table className="w-full text-sm border-collapse">
                                                        <tbody>
                                                            {data.map((row: string[], rIndex: number) => (
                                                                <tr key={rIndex} className={rIndex === 0 ? "bg-muted/50 font-medium" : ""}>
                                                                    {row.map((cell: string, cIndex: number) => (
                                                                        <td key={cIndex} className="border p-0 relative min-w-[100px]">
                                                                            <Input 
                                                                                value={cell}
                                                                                onChange={(e) => {
                                                                                    const newData = [...data];
                                                                                ...
                                                                                className="border-none shadow-none focus-visible:ring-0 rounded-none h-auto py-2 px-3 focus:bg-blue-50"
                                                                            />
                                                                        </td>
                                                                    ))}
                                                                    <td className="w-8 border-l bg-gray-50 text-center cursor-pointer hover:bg-red-50 text-gray-400 hover:text-red-500 transition-colors" onClick={() => removeRow(rIndex)}>
                                                ...
                                                <div className="flex gap-2">
                                                    <Button variant="outline" size="sm" onClick={addRow}><Plus className="h-3 w-3 mr-1" /> {t.editor.buttons.addRow}</Button>
                                                    <Button variant="outline" size="sm" onClick={addCol}><Plus className="h-3 w-3 mr-1" /> {t.editor.buttons.addCol}</Button>
                                                    <Button variant="outline" size="sm" onClick={() => removeCol(data[0].length - 1)} className="text-red-500 hover:text-red-600 hover:bg-red-50">
                                                        <Minus className="h-3 w-3 mr-1" /> {t.editor.buttons.deleteLastCol}
                                                    </Button>
                                                </div>
                                            </div>
                                        );
            ...
            <div className="sticky top-24 space-y-4">
                <Card>
                    <CardHeader className="pb-3">
                        <CardTitle className="text-sm font-medium text-gray-500 uppercase">{t.editor.componentLibrary}</CardTitle>
                    </CardHeader>
