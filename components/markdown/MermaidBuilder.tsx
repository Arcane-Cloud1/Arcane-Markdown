...
export function MermaidBuilder({ initialData, onUpdate }: MermaidBuilderProps) {
  ...
  const [links, setLinks] = useState<MermaidLink[]>(
    initialData?.links || [{ id: generateId(), sourceId: 'A', targetId: 'B' }],
  );
    ...
    <div className="space-y-4 rounded-md border bg-white p-4">
      ...
      <div className="flex items-center justify-between border-b pb-4">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <Label>布局方向</Label>
            <Select value={direction} onValueChange={(v: 'TD' | 'LR') => setDirection(v)}>
        ...
        <div className="flex gap-2">
          <Button size="sm" variant="outline" onClick={addNode}>
            <Plus className="mr-1 h-4 w-4" /> 添加节点
          </Button>
          <Button size="sm" variant="outline" onClick={addLink}>
            <ArrowRight className="mr-1 h-4 w-4" /> 添加连线
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        ...
        <div className="space-y-3">
          <h4 className="flex items-center gap-2 text-sm font-medium text-gray-500 uppercase">
            <Settings2 className="h-4 w-4" /> 节点列表 ({nodes.length})
          </h4>
                ...
                <div className="flex items-start gap-2">
                  <div className="grid flex-1 gap-2">
                    <div className="flex gap-2">
                      ...
                      <Select
                        value={node.shape}
                        onValueChange={(v: 'rect' | 'round' | 'diamond' | 'circle') =>
                          updateNode(node.id, { shape: v })
                        }
                      >
                    ...
                    <div className="font-mono text-xs text-gray-400">ID: {node.id}</div>
                  </div>
                  <Button
                    variant="ghost"
                    size="icon"
        ...
        <div className="space-y-3">
          <h4 className="flex items-center gap-2 text-sm font-medium text-gray-500 uppercase">
            <ArrowRight className="h-4 w-4" /> 连线列表 ({links.length})
          </h4>
          <div className="max-h-[300px] space-y-2 overflow-y-auto pr-2">
            {links.map((link) => (
                ...
                <div className="flex items-start gap-2">
                  <div className="grid flex-1 gap-2">
                    <div className="flex items-center gap-2">
                      <Select
                        value={link.sourceId}
                        onValueChange={(v) => updateLink(link.id, { sourceId: v })}
                      >
                      ...
                      <Select
                        value={link.targetId}
                        onValueChange={(v) => updateLink(link.id, { targetId: v })}
                      >
                        <SelectTrigger className="h-8 flex-1">
            ...
            {links.length === 0 && (
              <div className="rounded-md border-2 border-dashed py-8 text-center text-sm text-gray-400">
                暂无连线
              </div>
            )}
          </div>
